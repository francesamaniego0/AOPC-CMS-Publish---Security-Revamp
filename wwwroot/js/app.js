...
async function ShowPositionName() {
  ...
  setTimeout(function () {
    $.ajax({ url: "/Register/GetPosition", data: {}, type: "GET", datatype: "json" }).done(function (data) {
      ...
      for (var i = 0; i < data.length; i++) {
        $("#corp-position").append('<option value="' + data[i].id + '">' + data[i].name + "</option>");
      }
      //$.unblockUI();
    }).fail(function () {
      alert("There was an Error When Loading Data...");
    ...
    $.ajax(
      {
        ...
        for (var i = 0; i < data.length; i++) {
          $("#user-position").append('<option value="' + data[i].id + '">' + data[i].name + "</option>");
        }
        //$.unblockUI();
      }).fail(function () {
        alert("There was an Error When Loading Data...");
...
async function ShowCorporateDetails() {
           ...
           //     });
    // }, 100);
            if (DataTable.isDataTable('#_corporate-table')) {
                corporateListTable.destroy();
            }

            corporateListTable = new DataTable('#_corporate-table', {
                ajax: {
                    url: '/Corporate/GetCorporateList',
                ...
                columns: [
                    {
                        ...
                        render: function (data, type, row) {
                            var tdbuttons = ' <div>' +
                                "<div>" + "<p>" + data +
                                "</p>" + "<p>" + row.corporateName + "</p>" +
                                '<div class="actions"><a class="tbl-edit" data-id="' + row.id +
                                '" data-corporatename="' + row.corporateName +
                                '"data-address="' + row.address +
                                '"  data-cno="' + row.cNo +
                                '" data-eadd="' + row.emailAddress +
                                '"   data-description="' + row.description +
                                '"  data-tier="' + row.tier +
                                '" data-dstart="' + row.dateUsed +
