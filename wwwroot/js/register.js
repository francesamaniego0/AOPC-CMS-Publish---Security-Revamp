    ...
    $.ajax(
        {
            ...
            for (var i = 0; i < data.length; i++) {
                $("#positionFilter").append('<option value="' + data[i].id + '">' + data[i].name + "</option>");
            }
            //$.unblockUI();
        }).fail(function () {
    ...
    $.ajax({
        ...
        if (response.stats == "Success!") {
            ...
            .done(function (response) {
                ...
                if (response.stats == 'Registered Successfully' || response.stats == 'Updated Successfully') {
                    //aler("Success");
                    //$.ajax({
                    //    url: '/Register/UploadFile',
                    //    type: 'POST',
                    ...
                    notifyMsg('Error!', response.stats, 'yellow', 'fas fa-exclamation-triangle');
                    $("#empid").focus();
                }
                else {
